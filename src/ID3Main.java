...
public class ID3Main {
    ...
    public static void ID3(String filename) {
        List<List<String>> availableAttributes_car = new ArrayList<List<String>>();
        ArrayList<Instance> data_car = new ArrayList<Instance>();
        ArrayList<Metadata> metadata_car = new ArrayList<Metadata>();
        TreeToXml xml_car = new TreeToXml();
        //Get data from Data file and metadata_car(attributes and possible values)
        ID3Main.fetchData(data_car, metadata_car, "\\data\\" + filename + ".data", "\\data\\" + filename + ".metadata");
        availableAttributes_car = metadata_car.get(0).getListAttributes();
        ID3Main dt = new ID3Main();
        //Start building the tree, tree is stored in object dt.root
        dt.learn(data_car, metadata_car, availableAttributes_car);
        //pass dt.root and nt.root to method generateXmlFromNode to create XML
        xml_car.generateXmlFromNode(dt.root, filename);
        System.out.println();
        System.out.println("Decision Tree Successfully created");
